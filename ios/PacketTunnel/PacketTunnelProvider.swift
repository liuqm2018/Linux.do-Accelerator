...
final class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    override func startTunnel(options: [String: NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        // Certs AND the edited config are passed in via providerConfiguration,
        // so the app-installed CA and the proxy's server cert are the same bundle
        // and the extension honors the user's DoH/domain edits (no IPC, no App
        // Group needed). Falls back to the shared file, then bundledDefault.
        let proto = protocolConfiguration as? NETunnelProviderProtocol
        let providerConfig = proto?.providerConfiguration ?? [:]

        if let toml = providerConfig["config_toml"] as? String, !toml.isEmpty {
            config = ConfigStore.from(toml: toml)
            // The Rust core reads its DoH endpoints via ConfigStore.load() (the
            // extension's own file). Persist the delivered TOML there so the
            // proxy uses the user's edited DoH, not bundledDefault.
            ConfigStore.persist(toml: toml)
        } else {
            config = ConfigStore.load()
        }

        guard let caPem = providerConfig["ca_pem"] as? String,
              let certPem = providerConfig["server_cert_pem"] as? String,
              let keyPem = providerConfig["server_key_pem"] as? String,
              !caPem.isEmpty, !certPem.isEmpty, !keyPem.isEmpty else {
            os_log("missing cert PEMs in providerConfiguration", log: log, type: .error)
            completionHandler(NSError(domain: "io.linuxdo.accelerator", code: 3,
                                      userInfo: [NSLocalizedDescriptionKey: "缺少证书，请在 App 内先安装根证书"]))
            return
        }

        // Start the in-extension ECH proxy (binds config listen_host = 127.0.0.1).
        let bundle = RustCore.Bundle(caPem: caPem, serverCertPem: certPem, serverKeyPem: keyPem)
        if !core.startWithCerts(bundle) {
            let reason = RustCore.lastError() ?? "unknown"
            os_log("failed to start Rust ECH proxy: %{public}@", log: log, type: .error, reason)
            completionHandler(NSError(domain: "io.linuxdo.accelerator", code: 2,
                                      userInfo: [NSLocalizedDescriptionKey: "本地 ECH 代理启动失败：\(reason)"]))
            return
        }
